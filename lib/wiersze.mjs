...
function koszyki(rekordy, teraz) {
  ...
  for (const rekord of rekordy) {
    ...
    if (!Number.isFinite(idx) || idx >= terazIdx || terazIdx - idx > OKNO_GODZIN) continue;
 ...
 */
export async function dopiszWiersze(rekordy, csvy, teraz, opisz) {
  const wynik = { ...csvy };
  const opisane = new Set(
    Object.values(csvy).flatMap(czytajCsv).map(w => kluczKoszyka(w.data, w.godzina, w.repo)),
  );

  for (const [klucz, koszyk] of koszyki(rekordy, teraz)) {
